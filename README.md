...
| **US-10** | Registrar avance diario de obra | Como residente de obra, quiero registrar el porcentaje de avance físico del proyecto diariamente, para mantener la trazabilidad de la ejecución real frente al cronograma. | **Escenario 1 (Éxito):** <br>Dado que el residente accede al reporte de jornada, <br>Cuando digita un valor de 1 a 100 en el input Avance, selecciona la Fecha y redacta la Observación, <br>Entonces el dashboard actualiza visualmente la barra de progreso. <br><br>**Escenario 2 (Fallo):** <br>Dado que el residente intenta reportar el avance, <br>Cuando ingresa un porcentaje superior a "100", <br>Entonces el sistema subraya el campo en rojo e impide el guardado. | **EP02** |
...
| **US-12** | Visualizar Dashboard logístico | Como encargado de logística, quiero visualizar un resumen gráfico de requerimientos pendientes y presupuesto ejecutado, para tomar decisiones de compra rápidas sin tener que cruzar datos de Excel. | **Escenario 1 (Éxito):** <br>Dado que el profesional con rol de logística ingresa al sistema, <br>Cuando se carga el Dashboard, <br>Entonces se renderiza un gráfico de anillos comparando solicitudes y una tarjeta del presupuesto ejecutado. <br><br>**Escenario 2 (Alternativo):** <br>Dado que el mes recién inicia y no hay data histórica, <br>Cuando el profesional de logística entra a la vista, <br>Entonces el espacio del gráfico muestra una ilustración y el texto "Sin operaciones registradas". | **EP03** |
...
| **US-19** | Préstamo de herramientas | Como encargado de almacén, quiero controlar el préstamo de equipos a las cuadrillas para evitar pérdidas y gestionar el mantenimiento. | **Escenario 1 (Éxito):** <br>Dado que un operario requiere un equipo menor, <br>Cuando el almacenero busca el DNI en el buscador y asocia la herramienta, <br>Entonces el badge de la herramienta cambia visualmente a "En Uso". <br><br>**Escenario 2 (Fallo):** <br>Dado que se registra el retorno del equipo, <br>Cuando el almacenero selecciona el radio button "Devuelto con Daños" y no escribe justificación, <br>Entonces el botón de guardado no se habilita exigiendo detalles en el textarea. | **EP03** |
| **US-20** | Activación de Plugins | Como desarrollador, quiero habilitar módulos adicionales mediante la API de Kipu para extender las funciones básicas del sistema. | **Escenario 1 (Éxito):** <br>Dado que el administrador cuenta con un plugin compatible, <br>Cuando carga el archivo `.zip` en el input file y pulsa "Instalar", <br>Entonces el núcleo lo procesa y despliega una nueva sección funcional en el Sidebar. <br><br>**Escenario 2 (Fallo):** <br>Dado que el `.zip` cargado no cumple con la firma técnica del software, <br>Cuando pulsa "Instalar", <br>Entonces el sistema arroja un Modal Error con el trace log, denegando la instalación. | **EP05** |
| **US-21** | Registro de desperdicios y devoluciones | Como encargado de almacén, quiero registrar la cantidad de material dañado o sobrante por cada jornada, para ajustar las futuras compras y reducir el desperdicio acumulado. | **Escenario 1 (Éxito):** <br>Dado que existe material sobrante reutilizable, <br>Cuando el almacenero selecciona "Devolución a proveedor" en el menú desplegable y guarda, <br>Entonces el sistema descuenta el material y renderiza una nota de crédito a favor. <br><br>**Escenario 2 (Fallo):** <br>Dado que el almacenero intenta registrar un "Desperdicio", <br>Cuando deja la cantidad del desperdicio en blanco o igual a 0, <br>Entonces el formulario marca el error e impide registrar un volumen irreal. | **EP03** |
...
| **US-24** | Control de herramientas asignadas | Como encargado de almacén, quiero registrar las herramientas eléctricas asignadas (taladros, amoladoras) a un obrero específico para que sea el responsable de su devolución en buen estado. | **Escenario 1 (Éxito):** <br>Dado que se va a asignar una amoladora mediante su código serie, <br>Cuando el logístico la vincula al código del operario y pulsa Confirmar, <br>Entonces el estatus del equipo en la grilla cambia de "Disponible" a "Asignado". <br><br>**Escenario 2 (Fallo):** <br>Dado que el logístico intenta asignar la amoladora, <br>Cuando escanea o digita el código de un trabajador inactivo o no existente en la BD, <br>Entonces el input de búsqueda no le permite avanzar y muestra "Operario no hallado". | **EP03** |
| **US-25** | Monitoreo de temperatura y humedad para curado de concreto (IoT) | Como residente de obra, quiero recibir alertas automáticas cuando las condiciones ambientales no sean óptimas para el curado del concreto, para evitar fisuras y garantizar la calidad estructural. | **Escenario 1 (Éxito/Alerta):** <br>Dado que el gateway MQTT recibe el JSON del sensor termopar, <br>Cuando la propiedad `temperatura` excede el input configurado como 35°C en la vista, <br>Entonces el sistema notifica una alerta push inmediata de peligro en el curado al residente. <br><br>**Escenario 2 (Fallo de conexión IoT):** <br>Dado que el sensor físico se apaga o pierde conectividad, <br>Cuando el sistema no recibe payloads MQTT en un lapso mayor a 1 hora, <br>Entonces el estatus del sensor en el dashboard cambia de "En Línea" a "Offline (Rojo)". | **EP04** |
| **US-26** | Monitoreo de vibraciones en edificios colindantes (IoT) | Como supervisor de seguridad, quiero recibir alertas en tiempo real cuando los sensores de vibración detecten movimientos peligrosos durante excavaciones, para evitar daños estructurales a propiedades vecinas. | **Escenario 1 (Éxito/Bloqueo):** <br>Dado que el payload entrante de vibración arroja `7 mm/s`, <br>Cuando el backend compara con el umbral seguro de `5 mm/s`, <br>Entonces la pantalla del módulo de excavación se bloquea con un modal estricto de "Riesgo Sísmico". <br><br>**Escenario 2 (Liberación):** <br>Dado que la alerta bloquea las operaciones lógicas, <br>Cuando el supervisor revisa el campo, ingresa su contraseña y pulsa "Reanudar Actividad", <br>Entonces el modal se cierra y el log registra que autorizó el reinicio bajo su riesgo. | **EP04** |
| **US-27** | Sensor de nivel de materiales en tolvas (IoT) | Como encargado de almacén, quiero recibir alertas automáticas cuando el nivel de agregados (arena, piedra) en las tolvas esté por debajo del mínimo, para solicitar reabastecimiento antes de que se agote. | **Escenario 1 (Éxito/Ticket automático):** <br>Dado que la lectura del sensor láser indica `nivel: 12%`, <br>Cuando la plataforma evalúa que es inferior al 15% del límite configurado, <br>Entonces Kipu auto-genera y deposita en la bandeja logística un ticket de "Reabastecer Arena". <br><br>**Escenario 2 (Recarga automática):** <br>Dado que un camión vierte la arena y el sensor lee un salto al `80%`, <br>Cuando procesa el JSON, <br>Entonces la barra de stock en el módulo inventario sube sin que el encargado de almacén haya tecleado el ingreso. | **EP04** |
| **US-28** | Geolocalización de equipos de alto valor (IoT) | Como encargado de almacén, quiero rastrear la ubicación de equipos de alto valor (vibrador de concreto, estación total) mediante sensores GPS, para prevenir robos y optimizar su asignación en obra. | **Escenario 1 (Éxito):** <br>Dado que el GPS del teodolito emite telemetría constante, <br>Cuando el encargado ingresa a la vista de "Rastreo", <br>Entonces visualiza un mapa con un Pin dinámico desplazándose en vivo según la latitud y longitud. <br><br>**Escenario 2 (Fallo/Geocerca):** <br>Dado que el administrador dibujó en el visor un polígono "Límite de la Obra", <br>Cuando la coordenada entrante del GPS cae fuera de las fronteras de dicho polígono, <br>Entonces suena un Beep ininterrumpido en la web y envía SMS urgente alertando el robo/salida al encargado. | **EP04** |
| **US-29** | Landing page - Visualización de propuesta de valor | Como profesional del sector construcción que visita el sitio web, quiero conocer la propuesta de valor y funcionalidades principales de Kipu para entender cómo puede ayudarme en mis proyectos de construcción. | **Escenario 1 (Éxito):** <br>Dado que un visitante accede a la URL pública, <br>Cuando la página carga el Hero Section, <br>Entonces se muestra el título, la propuesta de valor y el botón primario "Registrarse". <br><br>**Escenario 2 (Navegación):** <br>Dado que el visitante desplaza la página hacia abajo, <br>Cuando llega a la sección de características, <br>Entonces se renderizan tarjetas gráficas (Cards) detallando RNC, inventarios y KPIs. | **EP06** |
| **US-30** | Landing page - Contenido para gestores operativos | Como arquitecto o ingeniero que visita el sitio, quiero encontrar información específica sobre cómo Kipu mejora la supervisión técnica y trazabilidad de obra para evaluar si se adapta a mis necesidades profesionales. | **Escenario 1 (Éxito):** <br>Dado que un gestor operativo hace scroll en la landing page, <br>Cuando navega a la vista "Para profesionales de obra", <br>Entonces visualiza contenido específico y el botón "Crear cuenta gratuita". <br><br>**Escenario 2 (Redirección):** <br>Dado que el gestor operativo hace clic en "Crear cuenta gratuita", <br>Cuando el sistema procesa el evento, <br>Entonces lo redirige al formulario de registro con el Radio Button "Gestor Operativo" preseleccionado. | **EP06** |
| **US-31** | Landing page - Contenido para equipos de logística | Como responsable de logística o administración que visita el sitio, quiero encontrar información específica sobre cómo Kipu centraliza pedidos y controla presupuestos para convencerme de su utilidad en mi área. | **Escenario 1 (Éxito):** <br>Dado que un profesional de logística lee la sección "Para equipos de logística", <br>Cuando hace clic en el botón "Solicitar demo", <br>Entonces se despliega un formulario exigiendo Nombre, Empresa y Correo. <br><br>**Escenario 2 (Fallo):** <br>Dado que el profesional de logística intenta solicitar la demo, <br>Cuando omite llenar el campo obligatorio "Correo Corporativo" y presiona Enviar, <br>Entonces el botón se deshabilita y pide completar el formulario. | **EP06** |
| **US-32** | Landing page - Misión y visión de PircaIndustries | Como visitante potencial, quiero conocer la misión y visión de PircaIndustries para evaluar la confianza y dirección de largo plazo del proyecto Kipu. | **Escenario 1 (Éxito):** <br>Dado que un visitante navega por la landing, <br>Cuando llega a la sección "Sobre nosotros", <br>Entonces visualiza misión y visión con redacción clara y consistente con la propuesta del producto. <br><br>**Escenario 2 (Navegación):** <br>Dado que el visitante desea volver al inicio, <br>Cuando hace clic en el botón "Conocer Kipu" de la sección institucional, <br>Entonces el sistema lo desplaza al Hero Section sin recargar la página. | **EP06** |
| **US-33** | Landing page - Canales de contacto y formulario | Como visitante interesado, quiero disponer de canales de contacto y un formulario para comunicarme con el equipo de Kipu sin salir del sitio. | **Escenario 1 (Éxito):** <br>Dado que un visitante accede a la sección de contacto, <br>Cuando completa Nombre, Correo y Mensaje y presiona "Enviar", <br>Entonces el sistema registra la solicitud y muestra una confirmación de envío exitoso. <br><br>**Escenario 2 (Fallo):** <br>Dado que el visitante intenta enviar el formulario, <br>Cuando deja vacío un campo obligatorio o digita un correo inválido, <br>Entonces el sistema marca los campos en error e impide el envío. | **EP06** |
| **US-34** | Landing page - Llamados a la acción de registro y demo | Como visitante que ya entendió la propuesta, quiero ver llamados a la acción visibles para registrarme o solicitar demo y avanzar al siguiente paso. | **Escenario 1 (Éxito):** <br>Dado que el visitante revisa distintas secciones de la landing, <br>Cuando encuentra un botón CTA de "Registrarse" o "Solicitar demo", <br>Entonces el sistema lo redirige al flujo correspondiente sin errores de navegación. <br><br>**Escenario 2 (Persistencia):** <br>Dado que el visitante llega al final de la página, <br>Cuando visualiza el bloque final de CTA, <br>Entonces se mantienen disponibles ambas opciones para conversión (registro y demo). | **EP06** |
| **US-35** | **Creación de nuevo proyecto de obra** | Como **Gestor Operativo de Obra**, quiero registrar una nueva obra en el sistema para iniciar su gestión operativa y trazabilidad. | **Escenario 1 (Éxito):** Dado que el gestor operativo accede al módulo de obras, Cuando completa el formulario con "Nombre del Proyecto", "Ubicación", "Cliente" y "Fecha de Inicio Estimada", Entonces el sistema crea el proyecto en estado "Planificación" y lo añade a su panel de control.<br><br>**Escenario 2 (Fallo/Duplicado):** Dado que el gestor operativo intenta crear un proyecto, Cuando ingresa un "Nombre del Proyecto" que ya existe activo en la base de datos, Entonces el sistema muestra una alerta indicando "El nombre del proyecto ya está en uso" e impide el registro. | EP02 |
| **US-36** | **Carga de presupuesto base** | Como **miembro del Equipo de Logística y Administración**, quiero cargar el presupuesto inicial para tener un tope financiero al validar compras y requerimientos. | **Escenario 1 (Éxito):** Dado que la obra está en fase de planificación, Cuando el administrador sube el archivo con el presupuesto estructurado o ingresa los montos totales, Entonces el sistema registra el presupuesto meta y lo refleja en el dashboard financiero con 0% ejecutado.<br><br>**Escenario 2 (Fallo/Monto Cero):** Dado que el equipo de administración intenta guardar el presupuesto base, Cuando el monto total ingresado es igual a 0, Entonces el sistema bloquea la acción y exige un valor mayor a cero para activar el control de costos operativo. | EP03 |
| **US-37** | **Creación de cronograma y partidas** | Como **Gestor Operativo de Obra**, quiero estructurar las partidas (tareas) y fechas límite para poder medir el Porcentaje de Plan Completado (PPC) durante la ejecución. | **Escenario 1 (Éxito):** Dado que el gestor operativo accede a la planificación del proyecto, Cuando añade las partidas especificando "Nombre de la tarea", "Fecha de inicio" y "Fecha de fin", Entonces el sistema las guarda, genera el cronograma visual y habilita estas partidas para el registro de avance diario.<br><br>**Escenario 2 (Fallo/Fechas Ilógicas):** Dado que el gestor operativo ingresa una nueva partida, Cuando selecciona una "Fecha de fin" que es anterior a la "Fecha de inicio" de la misma partida, Entonces el calendario resalta el error en rojo e impide guardar la tarea en el cronograma. | EP02 |
| **US-38** | Historial de cambios en planos | Como **Residente de Obra**, quiero visualizar el registro de versiones de un plano para entender la evolución del diseño. | **Dado que** el Residente accede al detalle de un plano, <br>**Cuando** selecciona "Ver Historial", <br>**Entonces** visualiza la lista cronológica con número de versión, fecha y responsable del cambio. | **EP07** |
| **US-39** | Subir nueva versión de plano | Como **Ingeniero de Diseño**, quiero cargar actualizaciones de los planos para que el equipo en campo trabaje con la información final. | **Dado que** el Ingeniero completa los datos de identificación del plano, <br>**Cuando** carga el nuevo archivo y confirma la acción, <br>**Entonces** el sistema incrementa el número de versión y archiva la anterior como obsoleta. | **EP07** |
| **US-40** | Descarga para uso Offline | Como **Residente de Obra**, quiero guardar planos en mi dispositivo para consultarlos en zonas sin conectividad (sótanos). | **Dado que** el profesional dispone de conexión temporal, <br>**Cuando** selecciona la opción de "Acceso local", <br>**Entonces** el sistema permite la consulta del archivo sin requerir flujo de datos activo posterior. | **EP07** |
| **US-41** | Registro de interferencias (Anotaciones) | Como **Supervisor de Obra**, quiero realizar anotaciones gráficas sobre el plano para reportar errores de diseño detectados en campo. | **Dado que** el Supervisor visualiza el plano digital, <br>**Cuando** marca un área y redacta una observación técnica, <br>**Entonces** el sistema guarda la capa de información vinculada al archivo para revisión del diseñador. | **EP07** |
| **US-42** | Visualización interactiva 3D | Como **Gestor Operativo**, quiero inspeccionar modelos tridimensionales para comprender cruces de instalaciones complejos. | **Dado que** existe un archivo de modelo digital, <br>**Cuando** el profesional activa la vista 3D, <br>**Entonces** el sistema permite rotar, hacer zoom y seccionar el modelo para inspección visual. | **EP07** |
| **US-43** | Filtro de planos por especialidad | Como **Gestor Operativo**, quiero segmentar la lista de archivos por rama técnica para localizar información crítica rápidamente. | **Dado que** la biblioteca contiene múltiples planos, <br>**Cuando** se selecciona una especialidad (ej. Sanitarias), <br>**Entonces** el sistema oculta los archivos que no pertenecen a dicha categoría. | **EP07** |
| **US-44** | Ordenamiento cronológico de planos | Como **Equipo de Logística**, quiero organizar los documentos por fecha de carga para identificar los últimos requerimientos técnicos. | **Dado que** el profesional visualiza la lista de planos, <br>**Cuando** selecciona el criterio de orden por fecha, <br>**Entonces** el sistema reubica los archivos más recientes en la parte superior de la vista. | **EP07** |
